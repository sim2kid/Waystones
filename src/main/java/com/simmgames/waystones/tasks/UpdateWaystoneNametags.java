...
public class UpdateWaystoneNametags extends BukkitRunnable {
    ...
    public void run() {
        ...
        if(data.players == null)
            return;
        for(int b = data.players.size()-1; b >= 0; b--)
        {
            WayPlayer p = data.players.get(b);
            if(p == null)
                return;
            if(p.LastNear == null)
            {
                p.LastNear = new ArrayList<>();
                return;
            }
            for(int z = p.LastNear.size()-1; z >= 0; z--) {
                Waystone wei = p.LastNear.get(z);
                if (wei != null) {
                    if (wei.hasNametag) {
                    ...
                    if (runTimes == 0 || runTimes == 10) {
                        ...
                        if (wei.timeLeftUntilFunctional() > 0) {
                            if (wei.timeLeftUntilFunctional() > 120) {
                                ...
                                loc.getWorld().playSound(loc, Sound.BLOCK_BEACON_AMBIENT, SoundCategory.BLOCKS, 1f, remap);
                            }
                        } else if (runTimes == 0) {
                            if (wei.RunReactivateEvent())
                                loc.getWorld().playSound(loc, Sound.BLOCK_BEACON_ACTIVATE, SoundCategory.BLOCKS, 6f, 0.5f);
                            loc.getWorld().playSound(loc, Sound.BLOCK_BEACON_AMBIENT, SoundCategory.BLOCKS, 1f, 0.6f);
                        }
                    }
